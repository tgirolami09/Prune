...
class TM {
  public:
    ...
    Move lastbestMove;
    int nbInARow;
    TM(int moveOverhead = 0, bool color = WHITE, int wtime = INT_MAX,
       int winc = INT_MAX, int btime = INT_MAX, int binc = INT_MAX,
       int movetime = INT_MAX, big hardnodes = MAX_BIG, big softnodes = MAX_BIG,
       int maxdepth = maxDepth);
    void init();
    bool shouldstop_hard(big nodes, timeMesure::time_point start);
    bool shouldstop_soft(big nodes, timeMesure::time_point start, int depth,
                         big bestMoveNodes, big lastUsedNodes, int evaldiff,
                         Move bestmove, const tunables &parameters,
                         bool verbose);
    sbig updateSoft(int depth, big bestMoveNodes, big totalNodes, int evaldiff,
