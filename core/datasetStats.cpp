   ...
   public:
    ...
    bool filter(const GameState& state, MoveInfo move, bool inCheck, int result) {
        ...
        if (filter_check && inCheck)
            return true;
        if (abs(move.score) > max_eval)
            return true;
        if (filter_tactical && state.board.isTactical(move.move))
            return true;
    ...
    for (int idFile = 1; idFile < argc; idFile++) {
        ...
        while (ftell(file) < file_size) {
            ...
            for (MoveInfo move : game.game) {
                ...
                // big dng = 0;
                // int nbmoves = movegen.generateLegalMoves(game.startPos,
                // inCheck, legalMoves, dng); bool islegal = false; for(int i=0;
                // i<nbmoves; i++){
                //     if(legalMoves[i].moveInfo == move.move.moveInfo)
                //         islegal = true;
                // }
                // if(!islegal){
                //     printf("\n%s %s\n", game.startPos.toFen().c_str(),
                //     move.move.to_str().c_str()); break;
                // }
                if (filter.filter(game.startPos, move, inCheck, game.result))
                    countFiltered++;
                else
                    countUnfiltered++;
