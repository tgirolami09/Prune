...
template <typename T, int maxi = -1, int mini = 0> class StatVar {
  public:
    ...
    void print(string start) const {
        ...
        printf("%s = %.2f ± %.2f (n=%ld)\n", start.c_str(), mean,
               sqrt(sumSquare / number - mean * mean), number);
        printf("  %ld <= x <= %ld\n", obsmin, obsmax);
        int cum[maxi - mini + 2];
        cum[0] = 0;
        ...
        for (double percent : {0.1, 1., 5., 10., 20., 25., 30., 40., 50., 60.,
            ...
            printf("    %.2f : %.2f (%.2f %.2f)\n", percent,
                   ((double)(cum[left] - search) * (left - 1 + mini) +
                    (double)(search - cum[left - 1]) * (left + mini)) /
                       (cum[left] - cum[left - 1]),
                   (double)cum[left] * 100 / cum[maxi - mini + 1],
                   (double)cum[left - 1] * 100 / cum[maxi - mini + 1]);
        }
    }
};
#endif