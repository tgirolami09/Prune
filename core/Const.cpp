...
__attribute__((constructor(101))) void init_lines() {
    {
        ...
        for (int i = 0; i < 64; i++) {
            ...
            big maskPos = ~(1ULL << i);
            bishop_empty[i] =
                clipped_diag[colP + rowP] ^ clipped_idiag[rowP - colP + 7];
            bishop_full[i] =
                mask_diag[colP + rowP] ^ mask_idiag[rowP - colP + 7];
        ...
        for (int col = 0; col < 8; col++) {
            ...
            for (int idDir = 0; idDir < 8; idDir++) {
                ...
                while (r >= 0 && r < 8 && c >= 0 && c < 8) {
                    int sq = (r * 8 + c);
                    mask |= 1ULL << sq;
                    directions[square][sq] =
                        mask; // line of 1 between square and sq
                    r += dirs[idDir][0];
                    c += dirs[idDir][1];
                }
                r = row + dirs[idDir][0];
                ...
                while (r >= 0 && r < 8 && c >= 0 && c < 8) {
                    int sq = (r * 8 + c);
                    fullDir[square][sq] =
                        mask; // line of 1 from square in the direction of sq
                    r += dirs[idDir][0];
                    c += dirs[idDir][1];
                }
            }
