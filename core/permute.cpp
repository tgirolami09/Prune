...
const auto unpacked = [] {
    ...
    for (int i = 0; i < simdSize * 2; i++) {
        res[i] = i % simdSize / LaneSize * 2 * LaneSize + i % LaneSize +
                 (i / simdSize) * LaneSize;
    }
    return res;
}();

int permute(int n) { return (n & ~mask) | unpacked[n & mask]; }

template <int input, int output, typename wtype> struct layer {
    alignas(64) wtype weights[input][output];
    alignas(64) int32_t bias[output];
};

template <bool isPermuted> struct inputlayer {
    alignas(64) int16_t
        psqweights[nbInputBuckets][isPermuted ? INPUT_SIZE : RawInputSize][L1];
...
template <bool isPermuted> struct nn {
...
int main(int argc, char **argv) {
    ...
    for (int ob = 0; ob < BUCKET; ob++) {
        ...
        memcpy(&nn_out->laterLayers[ob], &nn_in->laterLayers[ob],
               sizeof(nn_in->laterLayers[ob]));
    }
        ...
        for (int i = 0; i < RawInputSize; i++)
            for (int k = 0; k < L1; k++) {
                ...
                if (pieceType != KING || (col(pos) > 3 && color) ||
                    (col(pos) <= 3 &&
                     (inputBuckets[col(pos) | (row(pos) << 2)] != ib) ==
                         color)) {
                    nn_out->FT.psqweights[ib][i - 6 * 64 *
                                                      (pieceType == KING &&
                                                       color)][k] =
    ...
    for (int i = 0; i < THREAT_SIZE; i++)
        for (int k = 0; k < L1; k++) {
            nn_out->FT.threatweights[i][k] =
                nn_in->FT.threatweights[i][permute(k)];
        }
    for (int k = 0; k < L1; k++) {
    ...
    for (int i = 0; i < BUCKET; i++)
        fwrite(&nn_out->laterLayers[i], sizeof(nn_out->laterLayers[i]), 1,
               fout);
    fclose(fin);
    fclose(fout);
}