...
template <>
struct simd_register<8> {
    using type = int8x16_t;
};
template <>
struct simd_register<16> {
    using type = int16x8_t;
};
template <>
struct simd_register<32> {
    using type = int32x4_t;
};
template <int size>
using simd = typename simd_register<size>::type;
#else
...
inline uint32_t nonzero_mask(__m256i reg) {
    return _mm256_movemask_ps(_mm256_castsi256_ps(_mm256_cmpgt_epi32(reg, _mm256_setzero_si256())));
}
