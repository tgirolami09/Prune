...
class LegalMoveGenerator {
  private:
    ...
    void maskToMoves(int start, big mask, Move *moves, int &nbMoves,
                     int8_t piece, bool promotQueen = false);
    ...
    big pseudoLegalPawnMoves(int pawnPosition, big allPieces, int friendKingPos,
                             big moveMask = -1, big captureMask = -1,
                             big enemyPieces = -1, int enPassant = -1,
                             big enemyRooks = 0);
    ...
    int dealWithEnemyBishops(big enemyBishopPositions, big Pieces,
                             int friendKingPos);
    int dealWithEnemyRooks(big enemyRookPositions, big allPieces,
                           int friendKingPos);
    void dealWithEnemyKing(int enemyKingPos);
    template <bool IsWhite>
    void legalKingMoves(const GameState &state, Move *moves, int &nbMoves,
                        big allPieces, big captureMask = -1);
    template <bool IsWhite>
    void legalPawnMoves(big pawnMask, int lastDoublePawnPush, big moveMask,
                        big captureMask, Move *pawnMoves, int &nbMoves,
                        big allPieces, big enemyRooks,
                        bool promotQueen = false);
    void legalKnightMoves(big knightMask, big moveMask, big captureMask,
                          Move *knightMoves, int &nbMoves);
    void legalSlidingMoves(big moveMask, big captureMask, Move *slidingMoves,
                           int &nbMoves, big allPieces);
    template <bool IsWhite> bool initDangersImpl(const GameState &state);
    template <bool IsWhite, bool InCheck>
    int generateLegalMovesImpl(const GameState &state, bool &inCheck,
                               Move *legalMoves, big &dangerPositions,
                               bool onlyCapture);
    template <bool IsWhite> Move getLVAImpl(int posCapture, GameState &state);
  ...
  public:
    ...
    int generateLegalMoves(const GameState &state, bool &inCheck,
                           Move *legalMoves, big &dangerPositions,
