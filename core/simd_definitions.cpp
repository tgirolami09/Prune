...
int mysum(const simd<32> &x) { return vaddvq_s32(x); }
...
simd<8> simd8_add(const simd<8> &a, const simd<8> &b) { return vaddq_s8(a, b); }

simd<8> simd8_sub(const simd<8> &a, const simd<8> &b) { return vsubq_s8(a, b); }

simd<16> simdh8_16(const simdhalf &v) { return vmovl_s8(v); }
...
simd<32> simdint_dpbusd(const simd<32> &acc, const simd<8> &u,
    ...
    // Single-instruction unsigned x signed dot product, should work on M2 chips
    // and later
    ...
    const int16x8_t p_lo =
        vmulq_s16(vreinterpretq_s16_u16(vmovl_u8(vget_low_u8(uu))),
                  vmovl_s8(vget_low_s8(s)));
    const int16x8_t p_hi =
        vmulq_s16(vreinterpretq_s16_u16(vmovl_u8(vget_high_u8(uu))),
...
#else
simd<16> simdh8_16(const simdhalf &v) { return ADDMM(cvtepi8_epi16)(v); }
#endif
...
#else
    return ADDMM(add_epi32)(
        acc, ADDMM(madd_epi16)(ADDMM(maddubs_epi16)(u, s), simd16_set1(1)));
#endif
}
