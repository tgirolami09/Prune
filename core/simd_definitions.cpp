...
simd<16> simd16_shr(const simd<16>& a, int b) {
    const uint16x8_t u = vshlq_u16(vreinterpretq_u16_s16(a), vdupq_n_s16(static_cast<int16_t>(-b)));
...
simd<32> simdint_dpbusd(const simd<32>& acc, const simd<8>& u, const simd<8>& s) {
    ...
    const int16x8_t p_lo =
        vmulq_s16(vreinterpretq_s16_u16(vmovl_u8(vget_low_u8(uu))), vmovl_s8(vget_low_s8(s)));
    const int16x8_t p_hi =
        vmulq_s16(vreinterpretq_s16_u16(vmovl_u8(vget_high_u8(uu))), vmovl_s8(vget_high_s8(s)));
