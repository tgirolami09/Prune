...
void Layer1<input, output>::forward(const uint32_t* x, simd<32>* y, const SparseIterator& si) const{
    ...
    for(int nnz_idx = 0; nnz_idx < nnz_4; nnz_idx += 4){
        ...
        __builtin_prefetch(&weights[si.index(nnz_idx+8)*I8inI32*output/nb<8>]);
        __builtin_prefetch(&weights[si.index(nnz_idx+9)*I8inI32*output/nb<8>]);
        __builtin_prefetch(&weights[si.index(nnz_idx+10)*I8inI32*output/nb<8>]);
        __builtin_prefetch(&weights[si.index(nnz_idx+11)*I8inI32*output/nb<8>]);
