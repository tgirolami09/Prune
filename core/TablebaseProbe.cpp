...
bool TablebaseProbe::canProbe(const GameState& state, int nbMan) const {
    ...
    if (nbMan > (int)tbLargest || nbMan > probeLimit)
        return false;
...
int TablebaseProbe::rootFiltering(const GameState& state, rootMove* moves, int& nbMoves) const {
    ...
    int lowerbound;
    if (bestRank >= 900)
        wdl = TB_RESULT_WIN, lowerbound = 900;
    else if (bestRank >= 100)
        wdl = TB_RESULT_CURSED_WIN, lowerbound = 100;
    else if (bestRank > -100)
        wdl = TB_RESULT_DRAW, lowerbound = -100;
    else if (bestRank > -900)
        wdl = TB_RESULT_BLESSED_LOSS, lowerbound = -899;
    else
