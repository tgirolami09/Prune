...
TM::TM(
    ...
    int movetime,
    big hardnodes, big softnodes,
    int maxdepth
):
    moveOverhead(moveOverhead), colorstm(color),
    wtime(wtime), winc(winc), btime(btime), binc(binc), enabledtm(false),
    movetime(movetime),
...
void TM::init(){
    ...
    if(softnodes != MAX_BIG)enablednodes = true;
    if(movetime != INT_MAX){
        hardtime = movetime;
        softtime = hardtime;
        enabledtime = true;
...
bool TM::shouldstop_hard(big nodes, timeMesure::time_point start){
    if(enablednodes && nodes >= hardnodes)return true;
...
bool TM::shouldstop_soft(big nodes, timeMesure::time_point start, int depth, big bestMoveNodes, big lastUsedNodes, int evaldiff, Move bestmove, const tunables& parameters, bool verbose){
    if(enablednodes && nodes >= softnodes)return true;
    if(enabledtime){
        auto timenow = timeMesure::now()-start;
...
sbig TM::updateSoft(int depth, big bestMoveNodes, big totalNodes, int evaldiff, Move bestmove, const tunables& parameters, bool verbose){
    if(!enabledtm)return softtime;
