...
void HelpOrdering::bonusMove(int depth, Move move, bool c,
    ...
    if (state.board.isTactical(move)) {
        ...
        updateHistory(depth * parameters.mainHist.bonus,
                      history[c][move.from()][move.to()][src_atk][dst_atk]);
        ExpendedMove lastmove = state.getLastMove();
        updateHistory(depth * parameters.prevHist.bonus,
                      conthist[!c][lastmove.piece][lastmove.move.to()][c]
...
void HelpOrdering::malusMove(int depth, Move move, bool c,
    ...
    if (state.board.isTactical(move)) {
        ...
        bool src_atk = attacked & (1ULL << move.from());
        bool dst_atk = attacked & (1ULL << move.to());
        updateHistory(-depth * parameters.mainHist.malus,
                      history[c][move.from()][move.to()][src_atk][dst_atk]);
        ExpendedMove lastmove = state.getLastMove();
        updateHistory(-depth * parameters.prevHist.malus,
                      conthist[!c][lastmove.piece][lastmove.move.to()][c]
                              [state.getPiece(move.from())][move.to()]);
    }
}
...
void Order::init(bool c, Move movePriority, const HelpOrdering &history,
    ...
    const int value_pieces[7] = {history.parameters.pvalue,
                                 history.parameters.nvalue,
                                 history.parameters.bvalue,
                                 history.parameters.rvalue,
                                 history.parameters.qvalue,
                                 100000,
                                 0};
...
#ifdef DEBUG_MACRO
            int moveHistory = history.getHistoryScore<TunableHist::ORDER>(
                moves[i], state.friendlyColor(), state, dangerPositions);
...
Move Order::pop_max(int &flag) {
    if (pointer < nbPriority) {
        ...
        // Horizontal reduction (i think this also get the lowest index for the
        // max)
