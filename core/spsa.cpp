...
class internalState {
  public:
    ...
    int nbInts;
    internalState(int nbParams, int _nbInts)
        : state(nbParams, 1), firstState(nbParams, 1), nbInts(_nbInts) {}
    ...
    void moveToFirst() { firstState = state; }
};
...
class stateIter {
  public:
    ...
    double diffloss() { return (double)score; }
...
int main(int argc, char **argv) {
    if (argc == 1) {
        ...
        printf(
            "added params: in case of restarted spsa, take in account in the "
            "reading of the file that paramaters were added in place x1 x2 "
            "etc. all the x1... should be between \" \" (like \"1 2 3\")\n");
        return 0;
    }
    string book(argv[1]);
    ifstream file(book);
